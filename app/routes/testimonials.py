...
async def create_testimonial(testimonial: Testimonial):
    result = testimonials_collection.insert_one(testimonial.model_dump())
...
async def edit_testimonial(testimonial_id: str, testimonial_data: Testimonial):
    testimonials_collection.update_one({"_id": ObjectId(testimonial_id)}, {"$set": testimonial_data.model_dump()})
