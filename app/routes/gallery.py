...
async def get_all_gallery():
    galleries = list(gallery_collection.find({}, {"_id": 1, "gallery_title": 1, "images": 1}))
    return galleries
...
async def create_gallery(gallery: GalleryItem):
    result = gallery_collection.insert_one(gallery.dict())
    created_gallery = gallery_collection.find_one({"_id": result.inserted_id})
...
async def edit_gallery(gallery_id: str, gallery_data: GalleryItem):
    gallery_collection.update_one({"_id": ObjectId(gallery_id)}, {"$set": gallery_data.dict()})
    updated_gallery = gallery_collection.find_one({"_id": ObjectId(gallery_id)})
    if updated_gallery:
