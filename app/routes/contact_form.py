...
async def submit_contact_form(contact_form: ContactForm):
    result = contact_forms_collection.insert_one(contact_form.dict())
    created_contact_form = contact_forms_collection.find_one({"_id": result.inserted_id}, {"_id": 0})
...
async def get_all_contact_forms():
    contact_forms = list(contact_forms_collection.find({}, {"_id": 0}))
...
async def edit_contact_form(contact_id: str, contact_data: ContactForm):
    contact_forms_collection.update_one({"_id": ObjectId(contact_id)}, {"$set": contact_data.dict()})
